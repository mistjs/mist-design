...
@import '../lib/style/dark.less';`;
    const baseFileDir = join(DIST_DIR, (getMistBuildName() || 'MistUI') + '.less');
    const darkFileDir = join(DIST_DIR, (getMistBuildName() || 'MistUI') + '.dark.less');
...
export const UI = async (options: BuildOptions = { dir: 'src', name: 'MistUI' }) => {
  init(options);
  await copySource();
