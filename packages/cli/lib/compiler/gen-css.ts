import { IMPORT_STYLE_PATH_RE, IMPORT_STYLE_RE, replaceCSSImportCSS } from '../common/css';
import { existsSync, outputFileSync, readFileSync } from 'fs-extra';
import { dirname } from 'path';
import { COMPONENT_LESS_FILE, OUT_PUT_COMPONENTS_LESS_FILE } from '../common/constant';
  ...
  if (existsSync(filePath)) {
    ...
    const myCode = code.replace(IMPORT_STYLE_RE, str => {
      ...
      if (importCode && importCode.length > 0) {
        ...
        return str.replace(replaceCode, `.${replaceCode}/index.less`).replace('import', '@import');
      } else {
        return '';
      }
    });
